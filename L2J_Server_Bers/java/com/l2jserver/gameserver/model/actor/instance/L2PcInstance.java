...
import com.l2jserver.gameserver.model.entity.events.CTF;
import com.l2jserver.gameserver.model.entity.events.DM;
import com.l2jserver.gameserver.model.entity.events.TvT;
...
public final class L2PcInstance extends L2Playable
{
	...
	public boolean							inEventRaid			= false;

	
	/** TvT Engine parameters */
	public String							_teamNameTvT, _originalTitleTvT;
	public int								_originalTitleColorTvT, _originalNameColorTvT, _countTvTkills, _countTvTdies, _originalKarmaTvT;
	...
	public boolean							inEventCTF				= false, _haveFlagCTF = false;
	public Future<?>						_posCheckerCTF			= null;

	/** DM Engine parameters */
	public String							_originalTitleDM;
	public int								_originalNameColorDM, _originalTitleColorDM, _countDMkills, _originalKarmaDM;
	...
	*/
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (pk != null && pk.inEventTvT && inEventTvT)
			{
				if (TvT._teleport || TvT._started)
				{
					...
					{
						pk.sendMessage("You are a teamkiller! Team killing is not allowed, you will get death penalty!");
					...
					noblesse.getEffects(this, this);
					setTitle("Kills: " + ((L2PcInstance) pk)._countTvTkills);
					getAppearance().setVisibleTitle("Kills: " + ((L2PcInstance) pk)._countTvTkills);
					broadcastUserInfo();
					broadcastTitleInfo();
				}
			}
			...
			else if (inEventRaid)
			{
				...
				{
					sendMessage("You will be revived and teleported to team spot in 1 second!");
					ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
					{
						public void run()
						{
							teleToLocation((Raid._startX + Rnd.get(-(Raid._radius), +(Raid._radius))), (Raid._startY + Rnd.get(-(Raid._radius), +(Raid._radius))), Raid._startZ, false);
							doRevive();
						}
					}, 1000);
					noblesse.getEffects(this, this);
			...
			else if ((killer instanceof L2PcInstance && ((L2PcInstance) killer).inEventDM) && inEventDM)
			{
				if (DM._teleport || DM._started)
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
					{
						public void run()
						{
							teleToLocation((DM._playerX + Rnd.get(-(DM._radius), +(DM._radius))), (DM._playerY + Rnd.get(-(DM._radius), +(DM._radius))), DM._playerZ, false);
							doRevive();
						}
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		if (inEventCTF || inEventTvT || inEventDM || inEventRaid) return;
	...
	public void increasePvpKills(L2Character target)
	{
		...
		{
			if ((inEventTvT) || (inEventDM) || (inEventCTF) || (inEventRaid)) { return; }
	...
	public void updatePvPStatus(L2Character target)
	{
		...
		if (player_target == null)
			return;
		if ((player_target.inEventTvT) || (player_target.inEventDM) || (player_target.inEventCTF) || (player_target.inEventRaid)) return;
	...
	{
		if ((inEventTvT && TvT._started) || (Raid._started && inEventRaid) || (inEventDM && DM._started) || (inEventCTF && CTF._started)) return true;
	...
	public void doRevive()
	{
		...
		if ((inEventTvT && TvT._started && Config.TVT_REVIVE_RECOVERY) || (inEventCTF && CTF._started && Config.CTF_REVIVE_RECOVERY) || inEventDM && DM._started || inEventRaid && Raid._started)
		{
	...
	public L2PcInstance getActingPlayer()
	{
		...
		{
			if (isGM() || isCursedWeaponEquipped())
	
			{
				return false;
			}
	
			// Disabled until we get the correct stats.
			//if (getMaxHp() >= 15000 || getMaxMp() >= 15000 || getMaxCp() >= 15000 || getMDef(null, null) >= 15000 || getPDef(null) >= 15000 || getMAtk(null, null) >= 15000 || getPAtk(null) >= 15000)
			//{
			//	return true;
			//}
			for (L2ItemInstance item : getInventory().getItems())
			...
			{
				if (item.isWeapon() && item.getEnchantLevel() > Config.SERVER_MAX_ENCHANT || item.isArmor() && item.getEnchantLevel() > Config.SERVER_MAX_ENCHANT)
		...
		private boolean _pvpoff = false;
		
		public void setPvPOff(boolean nopvp)
	    {
			_pvpoff = nopvp;
	    }
		...
		{
			return ((TvT._started && inEventTvT) || (TvT._sitForced && inEventTvT) || (Raid._started && inEventRaid) || (Raid._sitForced && inEventRaid) || (DM._started && inEventDM) || (DM._sitForced && inEventDM) || (CTF._started && inEventCTF) || (CTF._sitForced && inEventCTF) && !isGM());
		}
	@Override
	...
	public static boolean checkSummonTargetStatus(L2Object target, L2PcInstance summonerChar)
	{
		...
		if (summonerChar.inEventCTF || summonerChar.inEventTvT || summonerChar.inEventDM || summonerChar.inEventRaid 
				|| targetChar.inEventCTF || targetChar.inEventTvT || targetChar.inEventDM || targetChar.inEventRaid)
		{
