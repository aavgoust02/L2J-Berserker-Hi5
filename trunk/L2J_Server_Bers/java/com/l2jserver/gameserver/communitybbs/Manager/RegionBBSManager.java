	...
	private void writeCommunityPages()
	{
		...
		for (int page : _onlinePlayers.keySet())
		{
			...
			StringUtil.append(htmlCode, "<html><body><br>" + "<table>" + trOpen + "<td align=left valign=top>Server Restarted: ", String.valueOf(GameServer.dateTimeServerStarted.getTime()), tdClose
					...
					+ "SP Rate: x", String.valueOf(Config.RATE_SP), tdClose + colSpacer + tdOpen + "Party SP Rate: x", String.valueOf(Config.RATE_SP
							* Config.RATE_PARTY_SP), tdClose + colSpacer + tdOpen + "SP Exponent: ", String.valueOf(Config.ALT_GAME_EXPONENT_SP), tdClose
							...
							+ trOpen + tdOpen, String.valueOf(getOnlineCount("gm")), " Player(s) Online</td>" + trClose);
					if (Config.SHOW_LEGEND)
						StringUtil.append(htmlCode, trOpen + tdOpen + "<font color=\"LEVEL\">GM</font> - <font color=\"00FF00\">Clan Leader</font> - <font color=\"FF00FF\">Cursed Weapon</font> - <font color=\"FF0000\">Karma</font> - <font color=\"999999\">Jailed</font>"
							+ tdClose + trClose);					
			...
			if (Config.BBS_SHOW_PLAYERLIST)
			{
				...
				for (L2PcInstance player : getOnlinePlayers(page))
				{
					...
					if (player.isGM())
						...
						StringUtil.append(htmlCode, "<font color=\"FF00FF\">" + player.getName() + "</font>");
					else if (player.getKarma() > 0 && Config.SHOW_KARMA_PLAYERS)
						StringUtil.append(htmlCode, "<font color=\"FF0000\">" + player.getName() + "</font>");
					else if (player.isInJail() && Config.SHOW_JAILED_PLAYERS)
						StringUtil.append(htmlCode, "<font color=\"999999\">" + player.getName() + "</font>");
					else
					{
			...
			StringUtil.append(htmlCode, "<html><body><br>" + "<table>" + trOpen + "<td align=left valign=top>Server Restarted: ", String.valueOf(GameServer.dateTimeServerStarted.getTime()), tdClose
					...
					+ "<td><img src=\"sek.cbui355\" width=600 height=1><br></td>"
					+ trClose
					+ trOpen + tdOpen, String.valueOf(getOnlineCount("pl")), " Player(s) Online</td>" + trClose);
					if (Config.SHOW_LEGEND)
						StringUtil.append(htmlCode, trOpen + tdOpen + "<font color=\"LEVEL\">GM</font> - <font color=\"00FF00\">Clan Leader</font> - <font color=\"FF00FF\">Cursed Weapon</font> - <font color=\"FF0000\">Karma</font> - <font color=\"999999\">Jailed</font>"
							+ tdClose + trClose);					
					htmlCode.append("</table>");
