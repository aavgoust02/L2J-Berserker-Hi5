...
public abstract class L2PlayableAI extends L2CharacterAI
{
	...
	protected void onIntentionAttack(L2Character target)
	{
		if (target instanceof L2Playable)
		{
			...
			if (target.getActingPlayer().isPvPOff()	&& !target.getActingPlayer().isInFunEvent() && !target.getActingPlayer().isInOlympiadMode() && !target.getActingPlayer().isCursedWeaponEquipped() && !target.getActingPlayer().isInsideZone(L2Character.ZONE_SIEGE) && !target.getActingPlayer().isInsideZone(L2Character.ZONE_PVP))
			{
				_actor.getActingPlayer().sendPacket(SystemMessageId.TARGET_IS_INCORRECT);
				clientActionFailed();
				return;
			...
			if (_actor.getActingPlayer().isPvPOff()	&& !_actor.getActingPlayer().isInFunEvent() && !_actor.getActingPlayer().isInOlympiadMode() && !_actor.getActingPlayer().isCursedWeaponEquipped() && !_actor.getActingPlayer().isInsideZone(L2Character.ZONE_SIEGE) && !_actor.getActingPlayer().isInsideZone(L2Character.ZONE_PVP))
			{
				_actor.getActingPlayer().sendPacket(SystemMessageId.TARGET_IS_INCORRECT);
				clientActionFailed();
				return;
